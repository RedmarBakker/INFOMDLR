...
from data import (build_dataset, create_cross_validation_sets,
                  z_norm, load, downsample, rest_set_names, motor_set_names,
                  math_set_names, memory_set_names)
...
# Downsampled data
# X, y = build_dataset(sfreq_new=500)
# print(f"Data shape : {X.shape}")

# Autoencoder data
# import the autoencoders:
model = 'intra' # Change to 'cross' if needed

match model:
    case 'intra':
        filepath = './models/autoencoder_intra_dropout+L2.keras'
    case 'cross':
        filepath = './models/autoencoder_cross_dropout+L2.keras'
    case _:
...
# Per sample; chunk the data
# Give the data to the autoencoder
# Glue the data and restore original format (samples, n_channels, n_measurements)
